...
from matrix_generator import (
    initial_diagonal_by_prime_fraction,
    initial_diag_matrix,
    rotation_matrix
)
...
def test_initial_diag_matrix_correct():
    matrix_size, norm_value = 10, 50

    matrix = initial_diag_matrix(matrix_size=matrix_size, norm_value=norm_value)
...
def test_initial_diagonal_by_prime_fraction_correct():
    ...
    resulted_matrix = initial_diagonal_by_prime_fraction(size=diag_size, norm_value=norm_value)
...
def test_rotation_matrix_correct():
    ...
    assert np.allclose(rot_matrix[1, 1], cos(angle), 10e-8)
